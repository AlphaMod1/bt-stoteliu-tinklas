...
public class TestIntersectionsOfRoutes {
	...
	public void testfindIntersectionsOfRoutes() {
		...
		assertEquals(testList1, target.findIntersectionsOfRoutes(new String[] {""}, db.getStoteles()));
		assertEquals(22, target.findIntersectionsOfRoutes(new String[] {"Autobusas 2G: Stotis - Santariškės"}, db.getStoteles()).size());
		assertEquals(0, target.findIntersectionsOfRoutes(new String[] {"autobusas 6g: žaliasis tiltas - parko g.",
				"Autobusas 49 Ateities g. - Didlaukio g. - Žalgirio g. - Lazdynai - Bukčiai"}, db.getStoteles()).size());
